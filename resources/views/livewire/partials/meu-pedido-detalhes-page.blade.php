<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto" style="padding-top: 90px ">
        ...
        <div class="p-4 md:p-5 flex gap-x-4">
          ...
          <div class="grow">
            ...
            <div class="mt-1 flex items-center gap-x-2">
              <div>{{ $fullName = $pedido->endereco->full_name; }}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="grow">
            ...
            <div class="mt-1 flex items-center gap-x-2">
              <h3 class="text-xl font-medium text-gray-800 dark:text-gray-200">
                {{ $pedido_itens[0]->created_at->format('d-m-y') }}
              </h3>
            </div>
          </div>
        </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mt-4">
      <div class="md:w-3/4">
        <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
          <table class="w-full">
            ...
            <tbody>
              ...
              <!--[if BLOCK]><![endif]-->
                @foreach ($pedido_itens as $itens)
                    <tr wire:key="53" wire:key="{{ $itens->id }}">
                     <td class="py-4">
                      <div class="flex items-center">
                        <span class="font-semibold">{{ $itens->produto->nome }}</span>
                      </div>
                    </td>
                    <td class="py-4">{{ Number::currency($itens->preco, 'kzs') }}</td>
                    <td class="py-4">
                      <span class="text-center w-8">{{ $itens->quantidade }}</span>
                    </td>
                    <td class="py-4">{{ Number::currency($itens->valor_total, 'kzs') }}</td>
                  </tr>
                @endforeach
        ...
        <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
          <h1 class="font-3xl font-bold text-slate-500 mb-3">Endereco de entrega</h1>
          <div class="flex justify-between items-center">
            <div>
              <p>{{ $endereco->endereco_rua }}, {{ $endereco->cidade }}, {{ $endereco->caixa_postal }}</p>
            </div>
            <div>
      ...
      <div class="md:w-1/4">
        <div class="bg-white rounded-lg shadow-md p-6">
          <h2 class="text-lg font-semibold mb-4">Sumário</h2>
          <div class="flex justify-between mb-2">
            <span>Subtotal</span>
            <span>{{ Number::currency($itens->valor_total, 'kzs') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Taxas</span>
            <span>{{ Number::currency(0, 'kzs') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Entrega</span>
            <span>{{ Number::currency(0, 'kzs') }}</span>
          ...
          <div class="flex justify-between mb-2">
            ...
            <span class="font-semibold">{{ Number::currency($itens->valor_total, 'kzs') }}</span>
          </div>
